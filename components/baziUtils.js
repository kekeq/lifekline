...
// 计算距离1900年1月1日的天数
const getDaysSince1900 = (year, month, day) => {
    let days = 0;
...
const calculateYearPillar = (year, month, day) => {
    // 立春通常在2月4日左右
...
// 计算月柱
const calculateMonthPillar = (year, month, day) => {
    // 获取年柱天干（考虑立春）
...
// 计算时柱
const calculateHourPillar = (dayPillar, hour, minute = 0) => {
    // 获取日柱天干
...
// 计算起运年龄
// 起运年龄 = (出生日到最近节气的天数) / 3
const calculateStartAge = (year, month, day, gender) => {
...
// 计算第一步大运
const calculateFirstDaYun = (monthPillar, startAge, gender, year, month, day) => {
    // 获取年柱来确定顺逆（考虑立春）
    const yearPillar = calculateYearPillar(year, month, day);
    const yearStem = yearPillar[0];
    const isYangStem = ['甲', '丙', '戊', '庚', '壬'].includes(yearStem);
