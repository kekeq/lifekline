...
const ImportDataMode: React.FC<ImportDataModeProps> = ({ onDataImport }) => {
    ...
    const generateUserPrompt = () => {
        ...
        return `=== User Input ===
性别：${genderStr}
出生：${baziInfo.birthYear}年${baziInfo.birthMonth}月${baziInfo.birthDay}日 ${baziInfo.birthHour}:${baziInfo.birthMinute} (阳历)
八字：${baziInfo.yearPillar} | ${baziInfo.monthPillar} | ${baziInfo.dayPillar} | ${baziInfo.hourPillar}
大运：起运${baziInfo.startAge}岁，首运[${baziInfo.firstDaYun}]，方向[${directionStr}]。

请根据以上八字和大运信息，按照系统指令生成1-100岁(虚岁)的人生流年K线数据和命理分析报告。`;
    };
